...
const AdminProducts = () => {
  ...
  const openCreateModal = () => {
    setIsEditMode(false);
    setFormDataState({ title: "", description: "", price: "", size: "", year: "", dimension: "", category: "", artistId: "", quantity: "" });
  ...
  const handleSubmit = async () => {
    const { title, description, price, size, year, dimension, category, artistId, quantity } = formDataState;
    if (!title || !description || !price || !size || !year || !dimension || !category || !artistId || !quantity) {
      alert("Alle felt må fylles ut, inkludert bilde hvis nytt.");
      return;
    }
    try {
      const formPayload = new FormData();
      Object.entries({ title, description, price, size, year, dimension, category, artistId, quantity }).forEach(([key, value]) => {
        formPayload.append(key, value);
      });
      if (selectedFile) formPayload.append("image", selectedFile);

      const url = isEditMode
        ? `${API_BASE_URL}/api/items/${selectedProduct._id}`
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/items/${selectedProduct._id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            const updated = await fetch(`${API_BASE_URL}/api/categories`)
      ...
      <section className="mt-8 max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center mb-4">
          ...
          className="bg-[#2A2A2A] text-white w-[40px] h-[40px] text-lg flex items-center justify-center rounded-lg border border-[#444] cursor-pointer hover:border-[#FFD700] transition duration-300"
            ...
            <button onClick={() => setIsCategoryModalOpen(true)} className="bg-[#2A2A2A] text-white px-3 py-2 rounded-lg border border-[#444] cursor-pointer hover:border-[#FFD700] transition">+</button>
          </div>
          <button onClick={openCreateModal} className="bg-[#FFD700] cursor-pointer text-black px-4 py-2 rounded-lg font-semibold">Legg til nytt produkt</button>
          ...
          {products.filter((p) => !activeCategory || p.category === activeCategory).map((product) => (
            <div key={product._id} onClick={() => setSelectedProduct(product)} className="cursor-pointer rounded-2xl bg-[#2A2A2A] overflow-hidden shadow-md border-2 border-[#2A2A2A] hover:border-[#FFD700] transition">
              <div className="h-40 flex items-center justify-center overflow-hidden p-2 bg-[#2A2A2A]">
                {product.imageUrl ? (<img src={product.imageUrl} alt={product.title} className="h-full w-full object-cover"/>) : (<p className="text-sm text-gray-500">Bilde kommer</p>)}
              </div>
              <div className="p-4">
      ...
      {selectedProduct && !isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
          <div className="bg-[#2A2A2A] p-6 rounded-lg shadow-lg w-[400px] relative max-h-[90vh] overflow-y-auto">
            <button className="absolute top-2 right-3 text-gray-400 hover:text-white text-2xl cursor-pointer" onClick={() => setSelectedProduct(null)}>&times;</button>
            <h2 className="text-2xl font-semibold text-white mb-4 text-center">{selectedProduct.title}</h2>
            {selectedProduct.imageUrl && (<div className="mb-4"><img src={selectedProduct.imageUrl} alt={selectedProduct.title} className="w-full max-h-[300px] object-contain rounded"/></div>)}
            ...
            <div className="flex justify-end space-x-2 mt-4">
              <button onClick={() => openEditModal(selectedProduct)} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold cursor-pointer">Rediger</button>
              <button onClick={handleDelete} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-semibold cursor-pointer">Slett</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
          <div className="bg-[#2A2A2A] p-6 rounded-lg shadow-lg w-96 max-h-[90vh] overflow-y-auto relative">
            ...
            <label className="block text-[#F5F5F5] mb-1">Antall</label>
            <input
            type="number"
            name="quantity"
            min="0"
